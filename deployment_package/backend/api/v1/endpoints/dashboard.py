...
def get_admin_dashboard_stats(
    ...
    try:
        ...
        total_events_result = db.execute(select(func.count(Event.id)))
        total_events = total_events_result.scalar() or 0
        
        # События за текущий месяц
        current_month_start = datetime.utcnow().replace(day=1, hour=0, minute=0, second=0, microsecond=0)
        events_this_month_result = db.execute(
        ...
        total_news_result = db.execute(select(func.count(News.id)))
        total_news = total_news_result.scalar() or 0
        
        # Новости за текущий месяц
        news_this_month_result = db.execute(
        ...
        users_by_role_result = db.execute(
        ...
        recent_users_result = db.execute(
