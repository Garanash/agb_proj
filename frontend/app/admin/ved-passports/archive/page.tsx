...
const AdminVEDPassportsArchivePage = () => {
  ...
  const fetchPassports = async () => {
    ...
    try {
      ...
      const response: any = await fetch(`${apiUrl}/api/v1/ved-passports/admin/archive/?${queryParams}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  if (isLoading || !user || user.role !== 'admin') {
    return (
      <PageLayout title="Архив паспортов ВЭД">
  ...
  return (
    <PageLayout title="Архив паспортов ВЭД">
      <div className="p-6 bg-white rounded-lg shadow-lg">
        {/* Заголовок */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-800 mb-2">Архив паспортов ВЭД</h1>
          <p className="text-gray-600">Просмотр и управление всеми паспортами ВЭД в системе</p>
        </div>
...
export default AdminVEDPassportsArchivePage;
